...
const Chat = () => {
  ...
  const parsePracticeSentence = (text) => {
    ...
    // Pattern 1: **"Sentence"** or "Sentence"
    const boldQuoteMatch = text.match(/\*\*"([^"]+)"\*\*/)
    if (boldQuoteMatch) {
      const sentence = boldQuoteMatch[1]
      // Try to extract romanization
  ...
  const handleTextSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success && result.response) {
        ...
        // Check if response contains a practice sentence
        const practiceSentence = parsePracticeSentence(result.response)
        if (practiceSentence) {
          setCurrentChallenge(practiceSentence)
        }
      } else {
      ...
      <div className="border-t border-white/10 p-4 bg-slate-900/50 backdrop-blur-xl">
        ...
        <form onSubmit={handleTextSubmit} className="flex items-center space-x-2">
          ...
          <button
            ...
            disabled={isLoading}
            className={`relative flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed ${
              isRecording
